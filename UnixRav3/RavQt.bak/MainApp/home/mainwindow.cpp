...
//        qDebug("logAction");
        ...
        if (mAppConfig->GetProductType() == 0) {
            if (mActived) {
                ...
                //QString text = QInputDialog::getText(NULL, "激活", "请输入序列号", QLineEdit::Normal, "XXXX-XXXX-XXXX-XXXX", &isOK);
				Active();
            }
        } else {
            Update(false, bincmd);
...
{
	CQActiveDialog dialog(this);

	if(dialog.exec() == QDialog::Accepted) 
	{
		int           ret;
		std::string   failed, err;
		std::string & str = dialog.GetActiveInfo();

		ret = Register(str);
		switch(ret)
		{
		case -13: err = u8"(SN无效)"; break;   // old sn.
		case -2:  err = u8"(授权文件格式错误)"; break;
		case -3:  err = u8"(授权文件解析错误)"; break;
		case -4:  err = u8"(授权文件无效)"; break;
		case -9:  err = u8"(无效授权文件)"; break;
		case -8:  err = u8"(无效产品授权)"; break;
		case -7:  err = u8"(授权过期)"; break;
		}

		if(ret != 0)
		{
			failed = u8"激活失败";
			if(!err.empty()) failed += " " + err;
		}

		QMessageBox::information(NULL, "提示", (ret==0)?"激活成功":failed.c_str(), QMessageBox::Ok, QMessageBox::Ok);
		RefreshRegisted();
	}
}

// ==========================================================================
CQActiveDialog::CQActiveDialog(QWidget * qbase)
	: m_layout(this)
{
	QLabel      * label1 = new QLabel(u8"请输入序列号: ", this);
	QLabel      * label2 = new QLabel(u8"请输入授权文件: ", this);

	m_qserial    = new QLineEdit(this);
	m_qlicense   = new QLineEdit(this);
	m_qbtnok     = new QDialogButtonBox(QDialogButtonBox::Ok);
	m_qbtncancel = new QDialogButtonBox(QDialogButtonBox::Cancel);
	m_qbtnopen   = new QDialogButtonBox(QDialogButtonBox::Open);

	m_qserial->setFixedWidth(200);      m_qserial->setPlaceholderText("XXXXX-XXXXX-XXXXX-XXXXX-XXXXX");
	m_qlicense->setFixedWidth(200);     m_qlicense->setPlaceholderText("xxx.lic");
	m_layout.addWidget(label1,   0, 0); m_layout.addWidget(m_qserial,    0, 1);
	m_layout.addWidget(label2,   1, 0); m_layout.addWidget(m_qlicense,   1, 1); m_layout.addWidget(m_qbtnopen, 1, 2);
	m_layout.addWidget(m_qbtnok, 2, 1); m_layout.addWidget(m_qbtncancel, 2, 2);

	m_qbtnok->button(QDialogButtonBox::Ok)->setText(u8"确认"); m_qbtncancel->button(QDialogButtonBox::Cancel)->setText(u8"取消"); m_qbtnopen->button(QDialogButtonBox::Open)->setText(u8"文件");
	m_layout.setSpacing(8); setWindowTitle(u8"激活"); setMaximumSize(420, 100);

	QObject::connect(m_qbtncancel, SIGNAL(rejected()), this, SLOT(reject()));
	QObject::connect(m_qbtnok,     SIGNAL(accepted()), this, SLOT(OnAccept()));
	QObject::connect(m_qbtnopen,   SIGNAL(clicked(QAbstractButton*)), this, SLOT(OnSelectLicFile()));
}

CQActiveDialog::~CQActiveDialog()
{
}

std::string & CQActiveDialog::GetActiveInfo()
{
	return m_strInfo;
}

void CQActiveDialog::OnSelectLicFile()
{
	QString qfile = QFileDialog::getOpenFileName(this, u8"aaa", ".", u8"*lic");
	std::string path = qfile.toStdString();
	if(!path.empty())  m_qlicense->setText(path.c_str());
}

void CQActiveDialog::OnAccept()
{
	std::string err, license;

	m_strInfo = m_qserial->text().toStdString();
	license   = m_qlicense->text().toStdString();
	if(m_strInfo.empty()) err = u8"序列号不能为空";
	else
	{
		switch(m_strInfo.length())
		{
		case 27:
			// xxxxxx-xxxxxx-xxxxxx-xxxxxx
			if(m_strInfo[6] != '-' || m_strInfo[13] != '-' || m_strInfo[20] != '-') err = "1";
			break;
		case 29:
			// xxxxx-xxxxx-xxxxx-xxxxx-xxxxx
			if(m_strInfo[5] != '-' || m_strInfo[11] != '-' || m_strInfo[17] != '-' || m_strInfo[23] != '-') err = "1";
			break;
		default:
			err = "1";
			break;
		}
		if(!err.empty()) err = u8"序列号格式错误";
	}

	if(err.empty() && m_strInfo.length() == 29)
	{
		if(license.empty()) err = u8"需要license文件";
		else
		{
			if(!Utils::Path::FileAvailed(license)) err = u8"license文件无效";
		}
	}

	if(!err.empty())
	{
		QMessageBox::information(this, u8"提示", err.c_str(), QMessageBox::Ok, QMessageBox::Ok);
		return;
	}

	if(!license.empty()) m_strInfo += "|"+license;
