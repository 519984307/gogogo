...
#include "ipwhitetable.h"
#include "iptables.h"
#include "common/pubdef.h"

#define COLUMN_NUMBER 2
#define ROW_NUMBER 6
#define DBTABLEIPW "ipwhite(ip text)"


//load close icon
IpWhiteItemModel::IpWhiteItemModel(QObject * parent) 
...
//init table
IpWhiteNameTable::IpWhiteNameTable(QWidget *parent)
    :QTableView(parent)
{
    ...
    *m_pImageDel = oripixmap.copy(0,0,oripixmap.width()/3,oripixmap.height());
    ...
    m_pDelegate = new IpWhiteIconDelegate;
	...
	//m_Stringdata = new IpTables("ipwhite(ip text)");
	m_Stringdata = new IpTables(DBTABLEIPW);

    m_nCurrentRow = -1;
	previouseindex = m_pModel->index(0,0);
	ischanged = false;

	m_Stringdata->ipsqldata->SqlQueryIp("ipwhite",1);
	WriteTable();
...
//disable tab key
bool IpWhiteIconDelegate::eventFilter(QObject *obj, QEvent *event) {
...
//paint close key
void IpWhiteIconDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex & index ) const
{
...
//when table data is changed,it will be called
void IpWhiteNameTable::ItemChangedHandle(QStandardItem *item) 
{
	QString str = item->text();
	int row = item->row();
	int size = m_Stringdata->m_IpWhiteData.size()-1;
	//qDebug() << item->text() << item->row() << item->column();
	if( !(str == "") && !m_Stringdata->IsIpValid(str))
	{
		//qDebug() << "error";
		QMessageBox::about(NULL, "提示", "ip格式错误,请重新输入。正确格式：192.168.1.1/192.168.1.1-192.168.1.100");
		return ;
	}
	...
	if(row > size && str != "")
	{
		m_Stringdata->m_IpWhiteData.push_back(str);
		ischanged = false;
	}
	...
	if(row < size)
	{
		m_Stringdata->m_IpWhiteData.erase(m_Stringdata->m_IpWhiteData.begin() + row);
		m_Stringdata->m_IpWhiteData.insert(m_Stringdata->m_IpWhiteData.begin() + row, str);
	...
	for (int i = 0; i < m_Stringdata->m_IpWhiteData.size(); i++) {
		qDebug() << m_Stringdata->m_IpWhiteData[i];
	}

}

//write table from db
void IpWhiteNameTable::WriteTable()
{
	for (int i = 0; i < m_Stringdata->ipsqldata->m_IpWhiteDB.size(); i++) {
		AddRowEdit();
		m_pModel->setItem(m_nCurrentRow,0,new QStandardItem(m_Stringdata->ipsqldata->m_IpWhiteDB[i]));
		m_Stringdata->m_IpWhiteData.push_back(m_Stringdata->ipsqldata->m_IpWhiteDB[i]);
		ischanged = false;
	}
...
void IpWhiteNameTable::AddRowEdit()
{
	m_nCurrentRow++;
...
void IpWhiteNameTable::mouseMoveEvent(QMouseEvent *event)
{
...
void IpWhiteNameTable::PressedHandle(QModelIndex index)
{
	m_Index = index;
	if(index.row() <= m_nCurrentRow)
	{
		switch(index.column())
		{
			case COLUMN_NUMBER-1:
				...
				if( index.row() <= m_Stringdata->m_IpWhiteData.size() -1)
					m_Stringdata->m_IpWhiteData.erase(m_Stringdata->m_IpWhiteData.begin() + index.row());
